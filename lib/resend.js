...
// Resolves the active mailer from DB settings, falling back to env vars.
// Returns null when no Resend API key is available anywhere.
async function resolveMailer() {
  let settings = null
  try {
    const { getEmailSettings } = await import('./email-settings')
    const res = await getEmailSettings()
    settings = res?.row || null
  } catch {
    // email_settings table not reachable — fall back to env
  }

  const key = settings?.resend_api_key || process.env.RESEND_API_KEY
  if (!key) return null

  const fromName = settings?.from_name || 'Khula Cafe'
  const fromEmail = settings?.from_email || 'orders@khulacafe.co.za'
  return {
...
// Sends an internal notification to staff (booking / contact). No-op unless a
...
export async function notifyStaff({ type, subject, html }) {
  const m = await resolveMailer()
  if (!m) return
  const to = m.settings?.notify_email
  if (!to) return
  if (type === 'booking' && m.settings?.notify_on_booking === false) return
  if (type === 'contact' && m.settings?.notify_on_contact === false) return

  await m.resend.emails.send({ from: m.from, to, subject, html: shell(html) })
}
...
// admin can see exactly why a send fails (e.g. unverified sender domain).
export async function sendTestEmail({ to }) {
  const m = await resolveMailer()
  if (!m) return { ok: false, error: 'No Resend API key found. Add one under Email Settings, or set the RESEND_API_KEY environment variable.' }
  if (!to) return { ok: false, error: 'Enter a recipient email address first.' }

  try {
    const { data, error } = await m.resend.emails.send({
      from: m.from,
      replyTo: m.replyTo,
      to,
      ...
      html: shell(`
        ...
        <p style="color:#555;line-height:1.7">Your Khula Cafe email settings are configured correctly. Booking, order, and contact-form emails will send from <strong>${m.from}</strong>.</p>
      `),
    })
