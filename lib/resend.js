...
// Records the outcome of every send so a silent failure becomes visible in
// the admin instead of vanishing. Never throws — logging must not break mail.
async function logDelivery({ recipients, subject, result, method }) {
  try {
    const { supabaseAdmin } = await import('./supabase-admin')
    await supabaseAdmin.from('email_log').insert({
      recipients: (recipients || []).join(', ').slice(0, 500),
...
// If the mail server refused the last few connections, stop hammering it for
// a while: each attempt otherwise burns ~12s waiting for a timeout. One real
// attempt is let through every PAUSE_MIN so sending resumes once it's fixed.
const PAUSE_AFTER = 3
const PAUSE_MIN = 10
const SKIP_PREFIX = 'Skipped:'
async function mailServerLooksDown() {
  try {
    const { supabaseAdmin } = await import('./supabase-admin')
    const { data } = await supabaseAdmin
      ...
      .eq('method', 'smtp')
      .not('error', 'like', `${SKIP_PREFIX}%`)
      .order('created_at', { ascending: false })
      .limit(PAUSE_AFTER)
    const rows = data || []
    if (rows.length < PAUSE_AFTER) return false
    const allConnectFails = rows.every(r => !r.ok && /timeout|ECONN|EHOSTUNREACH|ESOCKET|couldn.t connect/i.test(r.error || ''))
    const newest = new Date(rows[0].created_at).getTime()
    return allConnectFails && Date.now() - newest < PAUSE_MIN * 60000
  } catch { return false }
}

async function deliver(args, { force = false } = {}) {
  const recipients = recipientList(args?.to)
  const settings = args?.settings ?? (await loadSettings())
  const method = resolveMethod(settings)
  let result
  if (!force && method === 'smtp' && await mailServerLooksDown()) {
    result = { ok: false, error: `${SKIP_PREFIX} the mail server did not answer the last ${PAUSE_AFTER} attempts, so sending is paused. It is retried every ${PAUSE_MIN} minutes.` }
  } else {
    result = await deliverInner({ ...args, settings })
  }
  await logDelivery({ recipients, subject: args?.subject, result, method })
  return result
}

async function deliverInner({ to, subject, html, settings, replyTo }) {
  ...
  await deliver({
    ...
    html: shell(`
      ...
      <div style="text-align:center;margin:28px 0">
        <a href="${(process.env.NEXT_PUBLIC_SITE_URL || 'https://www.khulacafe.co.za')}/booking/${booking.id}"
           style="background:#f5c842;color:#0a0600;text-decoration:none;font-weight:700;padding:14px 30px;border-radius:30px;display:inline-block;letter-spacing:1px">Manage my booking</a>
        <p style="color:#888;font-size:12px;margin:10px 0 0">Change your date or cancel from there.</p>
      </div>
      <p style="color:#888;font-size:13px">Plans changed? Move your booking and the deposit comes with you. If you have to cancel because of unforeseen circumstances, your deposit is refunded within 7 working days. Prefer to talk? WhatsApp us on 061 489 4615.</p>
    `),
  })
}
...
export async function sendTestEmail({ to }) {
  ...
  const result = await deliver({
    ...
    `),
  }, { force: true }) // a test is a deliberate retry, so ignore any pause
  return result
}
