...
async function deliver({ to, subject, html, settings, replyTo }) {
  ...
  if (method === 'smtp') {
    ...
    try {
      const transport = nodemailer.createTransport({
        host: s.smtp_host,
        port,
        secure: s.smtp_secure ?? (port === 465),
        auth: { user: s.smtp_username, pass: s.smtp_password },
      })
      await transport.sendMail({ from, to, subject, html, replyTo: rt })
      return { ok: true }
    } catch (e) {
  ...
  if (method === 'resend') {
    ...
    try {
      ...
      const { data, error } = await resend.emails.send({ from, to, subject, html, replyTo: rt })
